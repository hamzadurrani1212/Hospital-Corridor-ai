...
def gen_frames():
    ...
    while True:
        ...
        if processor.running and (processor.yolo is None or processor.pose is None):
            # Show initializing frame
            init_frame = np.zeros((480, 640, 3), dtype=np.uint8)
            cv2.putText(init_frame, "INITIALIZING AI...", (180, 240), 
        ...
        if frame is None:
            # If camera is still booting up
            if processor.running:
                 boot_frame = np.zeros((480, 640, 3), dtype=np.uint8)
                 cv2.putText(boot_frame, "CONNECTING TO CAMERA...", (150, 240), 
                            cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 0), 2)
                 ret, buffer = cv2.imencode('.jpg', boot_frame)
                 yield (b'--frame\r\n'
                        b'Content-Type: image/jpeg\r\n\r\n' + buffer.tobytes() + b'\r\n')
